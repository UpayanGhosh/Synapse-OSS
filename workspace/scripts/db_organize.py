...
def organize():
    ...
    cursor.execute("SELECT id FROM documents WHERE length(content) > 100 AND instr(content, ' ') = 0")
    bad_ids = [row[0] for row in cursor.fetchall()]
    if bad_ids:
        print(f"Found {len(bad_ids)} binary garbage items. Deleting...")
        cursor.execute(f"DELETE FROM vec_items WHERE document_id IN ({','.join(['?']*len(bad_ids))})", bad_ids)
        cursor.execute(f"DELETE FROM documents WHERE id IN ({','.join(['?']*len(bad_ids))})", bad_ids)
        conn.commit()
