...
async def test_fresh_db_closes_memory_personality_loop(
    ...
    try:
        ...
        prior_pkg_deps = getattr(pkg, "_deps", None)
        had_pkg_deps = hasattr(pkg, "_deps")
        fake_deps = types.SimpleNamespace(memory_engine=types.SimpleNamespace(add_memory=_add_memory))
        sys.modules["sci_fi_dashboard._deps"] = fake_deps
        pkg._deps = fake_deps
        with (
        ...
        if "pkg" in locals():
            ...
            if had_pkg_deps:
                pkg._deps = prior_pkg_deps
            elif hasattr(pkg, "_deps"):
                delattr(pkg, "_deps")
        if archived.exists():
            archived.unlink()
