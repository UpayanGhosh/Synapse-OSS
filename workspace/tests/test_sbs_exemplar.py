...
class TestExemplarSelector:
    ...
    def populated_db(self, tmp_path):
        ...
        messages = [
            ...
            ("u1", now.isoformat(), "user",
             "hey bhai, let me implement the python api endpoint",
             "s1", None, 50, 9, 0, 0, 0.3, "mixed", "focused"),
            ("a1", now.isoformat(), "assistant",
             "sure bhai, let me build the fastapi route for you. chal dekh this approach",
             "s1", "u1", 60, 12, 0, 0, 0.5, "mixed", None),

            # Banglish pair
            ("u2", (now - timedelta(hours=12)).isoformat(), "user",
             "arey chai khaowa ghum lyadh",
             "s1", None, 25, 5, 0, 0, -0.2, "banglish", "tired"),
            ("a2", (now - timedelta(hours=12)).isoformat(), "assistant",
             "arey bhai, rest kor ektu! chai baniye dichi",
             "s1", "u2", 40, 7, 0, 0, 0.3, "banglish", None),

            # Personal/mood pair
            ("u3", (now - timedelta(days=2)).isoformat(), "user",
             "feeling stressed about the deadline, pressure is real",
             "s1", None, 45, 8, 0, 0, -0.4, "en", "stressed"),
            ("a3", (now - timedelta(days=2)).isoformat(), "assistant",
             "I understand, lets break it down into smaller tasks and prioritize",
             "s1", "u3", 55, 10, 0, 0, 0.2, "en", None),

            # Planning pair
            ("u4", (now - timedelta(days=5)).isoformat(), "user",
             "plan the todo schedule for this project goal",
             "s1", None, 40, 7, 0, 0, 0.0, "en", None),
            ("a4", (now - timedelta(days=5)).isoformat(), "assistant",
             "here is your project schedule with clear milestones and deliverables",
             "s1", "u4", 55, 9, 0, 0, 0.1, "en", None),

            # Playful pair
            ("u5", (now - timedelta(hours=6)).isoformat(), "user",
             "haha lol moja that was hilarious",
             "s1", None, 30, 6, 0, 0, 0.6, "mixed", "playful"),
            ("a5", (now - timedelta(hours=6)).isoformat(), "assistant",
             "haha glad you liked it! arey more jokes coming",
             "s1", "u5", 35, 7, 0, 0, 0.5, "mixed", None),
        ]
    ...
    def test_build_pairs_adjacency_fallback(self, tmp_path):
        ...
        now = datetime.now()
        messages = [
            ("u1", now.isoformat(), "user", "hello",
             "s1", None, 5, 1, 0, 0, 0.0, "en", None),
            ("a1", now.isoformat(), "assistant", "hi there",
             "s1", None, 8, 2, 0, 0, 0.0, "en", None),  # No response_to
        ]
        _create_db(db_path, messages)
