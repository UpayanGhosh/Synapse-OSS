...
def _fresh_channel_setup(monkeypatch, fake_deps, slack_cls=None):
    ...
    if slack_cls is not None:
        ...
        slack_module = types.ModuleType("channels.slack")
        slack_module.SlackChannel = slack_cls
        monkeypatch.setitem(sys.modules, "channels", channels_pkg)
        monkeypatch.setitem(sys.modules, "channels.slack", slack_module)
