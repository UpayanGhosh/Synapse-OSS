...
class MemoryEngine:
    ...
    def query(
        ...
        if with_graph and entities and self.graph_store:
            neighborhoods = []
            for ent in entities:
                # Assuming get_entity_neighborhood exists in the provided graph_store
        ...
        high_conf = [
            ...
            and (any(e.lower() in r["metadata"]["text"].lower() for e in entities) or not entities)
        ]

        if len(high_conf) >= limit:
            return {
                "results": [
                    {"content": x["metadata"]["text"], "score": x["combined_score"], "source": "qdrant_fast"}
        ...
        candidates = [
            {"id": r["id"], "text": r["metadata"]["text"], "meta": {"score": r["combined_score"]}}
            for r in q_results
        ]
