...
class BatchProcessor:
    ...
    def run(self, full_rebuild: bool = False):
        ...
        """
        print(f"[BATCH] Starting {'full rebuild' if full_rebuild else 'incremental'} at {datetime.now()}")
    ...
    def _update_vocabulary(self, messages: List[dict]):
        ...
        for word, data in word_registry.items():
            for pattern, normalized in COMPILED_BANGLISH.items():
                if pattern.search(word):
                    if normalized not in banglish_terms or \
                       data["effective_weight"] > banglish_terms[normalized]["weight"]:
        ...
        vocab["top_banglish"] = dict(sorted(
            banglish_terms.items(), 
    ...
    def _update_linguistic_profile(self, messages: List[dict]):
        ...
        current_style = {
            "banglish_ratio": round(sum(
                h["banglish_ratio"] * w for h, w in zip(style_history, weights)
            ) / total_weight, 3),
            "avg_message_length": round(sum(
                h["avg_message_length"] * w for h, w in zip(style_history, weights)
            ) / total_weight, 1),
            "emoji_frequency": round(sum(
                h["emoji_frequency"] * w for h, w in zip(style_history, weights)
            ) / total_weight, 3),
        }
        
        # Detect drift (compare current vs first half of history)
        if len(style_history) >= 4:
            old_banglish = sum(h["banglish_ratio"] for h in style_history[:len(style_history)//2]) / (len(style_history)//2)
            new_banglish = sum(h["banglish_ratio"] for h in style_history[len(style_history)//2:]) / (len(style_history) - len(style_history)//2)
            drift = new_banglish - old_banglish
            current_style["banglish_drift"] = round(drift, 3)
            current_style["drift_direction"] = "increasing" if drift > 0.05 else "decreasing" if drift < -0.05 else "stable"
    ...
    def _update_interaction_patterns(self, messages: List[dict]):
        ...
        if assistant_msgs:
            avg_response_len = sum(m.get("word_count", 50) for m in assistant_msgs) / len(assistant_msgs)
        else:
    ...
    def _update_domain_map(self, messages: List[dict]):
        ...
        DOMAIN_KEYWORDS = {
            "machine_learning": ["model", "training", "dataset", "neural", "ml", "ai", "llm", "transformer", "bert", "gpt"],
            "web_dev": ["react", "next", "api", "frontend", "backend", "css", "html", "node", "express"],
            "devops": ["docker", "deploy", "server", "nginx", "ci/cd", "pipeline", "kubernetes"],
            "python": ["python", "pip", "venv", "django", "flask", "fastapi", "pandas"],
            "personal": ["the_partner_nickname", "family", "health", "gym", "sleep", "food", "movie"],
        ...
        active_domains = sorted(
            domain_interests.items(),
            key=lambda x: x[1]["recent_mentions"],
            reverse=True
        )
    ...
    def _reselect_exemplars(self):
        ...
        self.profile_mgr.save_layer("exemplars", {
            "pairs": exemplars,
            "count": len(exemplars),
