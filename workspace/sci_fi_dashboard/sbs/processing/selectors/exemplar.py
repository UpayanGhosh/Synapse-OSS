...
class ExemplarSelector:
    ...
    def _build_pairs(self) -> List[Dict]:
        ...
        for msg in messages:
            if msg["role"] == "user" and msg["msg_id"] in response_map:
                ...
                pair_id = hashlib.md5(
                    (msg["msg_id"] + response["msg_id"]).encode()
                ).hexdigest()[:12]
    ...
    def _score_pairs(self, pairs: List[Dict]) -> List[Dict]:
        ...
        for pair in pairs:
            ...
            pair["scores"] = {
                ...
                "composite": round(
                    length_score * recency_score * lang_score * mood_score, 3
                )
            }
    ...
    def _select_recent_quality(self, pairs, used_ids, count) -> List[Dict]:
        ...
        candidates = [
            p for p in pairs 
            if p["timestamp"] > cutoff and p["pair_id"] not in used_ids
        ]
        ...
        for pair in pairs:
            ...
            for topic, keywords in TOPIC_KEYWORDS.items():
                if any(kw in text for kw in keywords):
                    if topic not in topic_best or pair["scores"]["composite"] > topic_best[topic]["scores"]["composite"]:
    ...
    def _select_mood_diverse(self, pairs, used_ids, count) -> List[Dict]:
        ...
        for pair in pairs:
            ...
            if mood and (mood not in mood_best or pair["scores"]["composite"] > mood_best[mood]["scores"]["composite"]):
                mood_best[mood] = pair
