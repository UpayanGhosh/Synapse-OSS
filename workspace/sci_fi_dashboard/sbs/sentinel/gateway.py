...
from .manifest import (
    ProtectionLevel, CRITICAL_FILES, CRITICAL_DIRECTORIES,
    PROTECTED_FILES, WRITABLE_ZONES, FORBIDDEN_OPERATIONS
)
...
class Sentinel:
    ...
    def __init__(self, project_root: Path, audit_dir: Path = None):
        ...
        self._critical_abs = {
            (self.project_root / f).resolve() for f in CRITICAL_FILES
        }
        self._critical_dirs_abs = {
            (self.project_root / d).resolve() for d in CRITICAL_DIRECTORIES
        }
        self._protected_abs = {
            (self.project_root / f).resolve() for f in PROTECTED_FILES
        }
        self._writable_abs = {
            (self.project_root / z).resolve() for z in WRITABLE_ZONES
        }
        
        # Compute integrity hash of manifest itself (detect tampering)
        self._manifest_hash = self._compute_manifest_hash()
        
        self.audit.log_event("SENTINEL_INIT", {
    ...
    def check_access(
        ...
        if not self._verify_manifest_integrity():
            self.audit.log_event("MANIFEST_TAMPERING_DETECTED", {
        ...
        try:
            ...
            raise SentinelError(f"Invalid path: {path}")
        ...
        if decision:
            ...
            self.audit.log_denial(str(resolved), operation, 
                                   f"Protection level: {level.value}")
            raise SentinelError(
    ...
    def safe_write(self, path: str, content: str, agent_context: str = "") -> bool:
        ...
        if resolved.exists():
            ...
            self.audit.log_event("PRE_WRITE_BACKUP", {
                "path": str(resolved),
    ...
    def safe_delete(self, path: str, agent_context: str = "") -> bool:
        ...
        if resolved.exists():
            ...
            if resolved.is_file():
                ...
                self.audit.log_event("PRE_DELETE_ARCHIVE", {
                    "path": str(resolved),
        ...
        for forbidden in FORBIDDEN_OPERATIONS:
            if forbidden.lower() in op_lower:
                self.audit.log_denial(
                    "CODE_EXECUTION", 
                    operation[:100],
                    f"Forbidden operation pattern: {forbidden}"
                )
                raise SentinelError(
