...
def vacuum_sbs(data_dir: str = "./data", retain_days: int = 30, keep_versions: int = 10):
    ...
    if profiles_archive.exists():
        versions = sorted([d for d in profiles_archive.iterdir() if d.is_dir()], key=lambda x: x.name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-dir", type=str, default="./jarvis_data/the_creator", help="Path to SBS data directory")
    parser.add_argument("--retain", type=int, default=30, help="Days of logs to keep in active DB (N/A yet)")
