...
BANGLISH_MARKERS = {
    'the_brother', 'bhalobasha', 'the_partner_nickname', 'accha', 'arrey', 'arey', 'hain',
    'toh', 'kor', 'koro', 'bolbo', 'bolo', 'dekhi', 'dekhbo', 'jabo',
    'asho', 'eso', 'ektu', 'kichu', 'keno', 'kothay', 'ki', 'kemon',
    'bhalo', 'khub', 'thik', 'achha', 'achis', 'korchis', 'korchi',
    'porchi', 'khacchi', 'ghumacchi', 'shuyo', 'ghumiye', 'joldi',
    'taratari', 'ekhon', 'pore', 'kal', 'ajke', 'dara', 'jawai',
    'sera', 'sotti', 'apon', 'amader', 'toder', 'oder', 'tai',
    'kintu', 'tobuo', 'jemon', 'temon', 'shob', 'protidin', 'majhe',
    'dorkar', 'lagbe', 'parbe', 'hobe', 'korbe', 'bolbe', 'jabe',
    'nebe', 'debo', 'rakhbo', 'pathiye', 'diyechi', 'korlam',
    'bollam', 'gelam', 'ashlam', 'shunlam', 'bujhte', 'perechi',
    'noshto', 'paglami', 'jhamela', 'chap', 'kaaj', 'somoy',
    'shanti', 'shundor', 'sundor', 'aram', 'ghum', 'ghumao',
    'biri', 'jiggasha', 'samle', 'bojh', 'chesta', 'hoye',
    'shobi', 'shobar', 'amake', 'toke', 'oke', 'oder',
    'yaar', 'dada', 'didi', 'mashi', 'kaku', 'mama',
    'matha', 'paglami', 'niye', 'koreche', 'korche', 'bolche',
}

TECH_JARGON = {
    'fastapi', 'python', 'jwt', 'api', 'backend', 'docker', 'kubernetes',
    'sqlite', 'qdrant', 'ollama', 'gemini', 'openai', 'llm', 'rag',
    'vector', 'embedding', 'model', 'deploy', 'server', 'uvicorn',
    'pydantic', 'sqlalchemy', 'alembic', 'celery', 'redis',
    'git', 'github', 'npm', 'pip', 'homebrew', 'terminal',
    'cron', 'webhook', 'endpoint', 'token', 'oauth', 'ssl',
}
...
def analyze_style(jarvis_messages: List[str]) -> dict:
    ...
    stopwords = {'the', 'a', 'an', 'is', 'it', 'in', 'to', 'for', 'of', 'and',
                 'on', 'that', 'this', 'you', 'i', 'me', 'my', 'we', 'our',
                 'your', 'he', 'she', 'they', 'them', 'his', 'her', 'its',
                 'be', 'are', 'was', 'were', 'been', 'have', 'has', 'had',
                 'do', 'does', 'did', 'will', 'would', 'can', 'could', 'may',
                 'might', 'shall', 'should', 'if', 'but', 'or', 'not', 'no',
                 'so', 'up', 'out', 'about', 'just', 'with', 'from', 'at',
                 'by', 'as', 'all', 'what', 'when', 'how', 'who', 'which',
                 'there', 'here', 'than', 'then', 'also', 'very', 'much',
                 'll', 've', 't', 's', 'd', 're', 'don', 'didn', 'won',
                 'get', 'got', 'make', 'like', 'know', 'want', 'need'}
    
    top_words = [w for w, _ in word_counter.most_common(100) if w not in stopwords and len(w) > 2][:30]
    ...
    for msg in jarvis_messages:
        ...
        fl = first_line.lower()
        if any(g in fl for g in ['hey', 'yo ', 'hi ', 'hello', 'good morning', 'good night', 'the_brother', 'the_partner_nickname', 'sup']):
            greetings.append(first_line)
        
        # Closing detection
        ll = last_line.lower()
        if any(c in ll for c in ['👊', '🚀', '💪', 'take care', 'good night', 'sleep well', 'ready when', 'let me know']):
    ...
    return {
        ...
        'top_words': top_words,
        'banglish_words': sorted(list(banglish_found)),
        'tech_jargon': sorted(list(tech_found)),
...
def detect_topic(text: str) -> str:
    ...
    t = text.lower()
    if any(w in t for w in ['job', 'resume', 'recruit', 'salary', 'lpa', 'career', 'yourworkplace', 'interview']):
        return 'career'
    if any(w in t for w in ['the_partner', 'the_partner_nickname', 'relationship', 'valentine', 'date', 'princess', 'romantic']):
        return 'relationship'
    if any(w in t for w in ['game', 'gaming', 'sims', 'elden', 'valorant', 'aniimo', 'palworld', 'overwatch']):
        return 'gaming'
    if any(w in t for w in ['python', 'fastapi', 'code', 'deploy', 'server', 'api', 'model', 'embedding', 'sqlite']):
        return 'tech'
    ...
    if any(w in t for w in ['email', 'gmail', 'himalaya', 'browser', 'cron', 'download']):
        return 'tasks'
    if any(w in t for w in ['sad', 'happy', 'upset', 'angry', 'depressed', 'frustrated', 'stressed']):
...
def select_best_examples(pairs: List[ConversationPair], n: int = 12) -> List[Dict[str, str]]:
    ...
    for pair, score, topic in scored:
        if len(selected) >= n:
            break
...
def build_persona_profile(
    ...
    print(f"   Style: avg_len={style.get('avg_message_length', 0)}, "
    ...
    print(f"   Selected {len(examples)} few-shot examples across topics: "
