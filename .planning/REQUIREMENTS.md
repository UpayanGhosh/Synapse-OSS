...
- [x] **ENC-01**: App boots on Windows without UnicodeEncodeError (all emoji in workspace/*.py replaced with ASCII tags)
- [x] **ENC-02**: Fix covers all affected files in one pass (not just smart_entity.py — grep confirms zero non-ASCII in print/log statements)

### Optional Ollama

- [x] **OLL-01**: App starts successfully when Ollama is not installed (module-level import guard in memory_engine.py)
- [x] **OLL-02**: Embedding falls back to sentence-transformers (all-MiniLM-L6-v2) when Ollama is unavailable
- [x] **OLL-03**: Startup prints a clear message listing which features are unavailable when Ollama is not detected
- [x] **OLL-04**: Onboarding scripts (synapse_onboard.bat and .sh) treat Ollama as optional, not required
...
- [x] **BRW-04**: `tools.py` browser abstraction dispatches to Crawl4AI or Playwright based on OS, with identical `search_web(url)` interface
