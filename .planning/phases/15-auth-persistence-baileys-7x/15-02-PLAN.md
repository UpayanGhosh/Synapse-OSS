...
threat_model:
  ...
  stride:
    - id: "T-15-02"
      category: "Tampering"
      component: "maybeRestoreCredsFromBackup"
      disposition: "mitigate"
      mitigation: "Two JSON.parse gates — first on current creds.json (bail if valid → no restore needed), second on backup before copyFileSync (never restore garbage). Parse failures caught silently so Baileys falls through to fresh-pair, not crash."
    - id: "T-15-07"
...
|-----------|----------|-----------|-------------|-----------------|
| T-15-02 | Tampering | maybeRestoreCredsFromBackup | mitigate | Two JSON.parse gates — (1) skip restore when current creds valid, (2) validate backup before copying. Invalid backup → no restore, fall through to re-pair. |
