...
threat_model:
  ...
  stride:
    ...
    - id: "T-15-16"
      ...
      disposition: "mitigate"
      mitigation: "user_id_alt is a PN — MUST be redacted by Phase 13 `redact_identifier()` wherever logged. Bridge itself only forwards; Python-side logs apply redaction at formatter boundary. Add user_id_alt to the Phase 13 PII allowlist keys so existing redaction filters catch it."
    - id: "T-15-17"
      category: "Tampering"
...
Nothing else in the bridge contract changes.
</interfaces>
</context>
...
<task type="auto" tdd="true">
  <name>Task 1: Update extractPayload to emit user_id_alt on inbound messages (BAIL-04)</name>
  <files>baileys-bridge/index.js, baileys-bridge/test/extract_payload.test.js</files>
...
**Step 6 — Python side additive tolerance check**: the Python webhook handler in `workspace/sci_fi_dashboard/routes/whatsapp.py` ingests the payload. Since `user_id_alt` is a new optional key, confirm `WhatsAppMessage` Pydantic model accepts the extra field without rejecting:
```bash
...
If Pydantic rejects with `extra='forbid'`, patch the model to either (a) add an explicit `user_id_alt: str | None = None` field OR (b) set `model_config = ConfigDict(extra='allow')`. Document the chosen approach in the SUMMARY. Default recommendation: add the explicit field — safer than opening up `extra='allow'`.
  ...
  <verify>
    <automated>cd baileys-bridge &amp;&amp; node --test test/extract_payload.test.js 2>&amp;1 | grep -E "pass|fail" | head -10 &amp;&amp; grep -q "user_id_alt" index.js</automated>
  </verify>
  ...
  <done>
    ...
    - `baileys-bridge/index.js` exports `{ extractPayload }` when imported; `app.listen()` + `startSocket()` guarded so tests can import without side effects
...
| Logs containing user_id_alt | user_id_alt is typically a PN — MUST go through Phase 13 `redact_identifier()` filter. Bridge forwards raw; Python formatter redacts. |
...
| T-15-16 | Information Disclosure | user_id_alt PN appearing in logs | mitigate | user_id_alt values are phone numbers. Python-side structured-log formatter (Phase 13 `JsonFormatter` + `redact_identifier()`) applies at emit time. Bridge-side Node logs only show authDir + event name, never user_id_alt value. |
