...
must_haves:
  truths:
    ...
    - "The search provider is swappable via configuration — DDGS is the default but the interface supports alternatives"
...
BROWSE-05 (source URLs in results). The search provider is designed to be swappable via
configuration, with DDGS as the zero-cost default.
