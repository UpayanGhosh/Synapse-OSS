...
must_haves:
  truths:
    ...
    - "Wizard asks 3 SBS persona questions after provider/channel setup"
    - "SBS profile layers (linguistic, domain, interaction) are written from wizard answers"
  ...
  artifacts:
    ...
    - path: "workspace/cli/sbs_profile_init.py"
      ...
      exports: ["initialize_sbs_from_wizard", "STYLE_CHOICES", "INTEREST_CHOICES", "PRIVACY_CHOICES"]
  ...
  key_links:
    ...
    - from: "workspace/cli/sbs_profile_init.py"
      ...
      via: "ProfileManager.save_layer() for linguistic, domain, interaction"
      pattern: "mgr\\.save_layer"
---

<objective>
Create the `setup` command entrypoint, SBS persona question step, and WhatsApp import offer.
...
history import. This is the core user-facing change of Phase 4.

Output: 4 files created/modified — `__main__.py`, `synapse_cli.py` (setup command),
...
Function `initialize_sbs_from_wizard(answers: dict, data_root: Path) -> None`:
  ...
  - For `domain` layer: `load_layer("domain")`, set `["interests"][topic] = 1.0` for each topic in `answers.get("interests", [])`, then `save_layer("domain", data)`
...
Note: `verify_steps.py` will be created in Plan 04-02. The import is deferred (inside `if verify:`) so it only fails if `--verify` is actually used before Plan 02 ships. This allows parallel execution.
  ...
  <verify>
    <automated>cd C:/Users/upayan.ghosh/personal/Synapse-OSS/workspace && python -c "from cli.sbs_profile_init import initialize_sbs_from_wizard, STYLE_CHOICES, INTEREST_CHOICES, PRIVACY_CHOICES; print('sbs_profile_init OK')" && python -c "from synapse_cli import app; print('synapse_cli OK')" && python __main__.py --help</automated>
  </verify>
  <done>
...
3. Ask interests via `prompter.multiselect()`:
...
4. Ask privacy sensitivity via `prompter.select()`:
...
5. Map display values to internal values using the `STYLE_DISPLAY_MAP` and `PRIVACY_DISPLAY_MAP` from `sbs_profile_init`
6. Map interest display values to lowercase (e.g., "Technology" -> "technology")
7. Call `initialize_sbs_from_wizard(answers_dict, data_root)` inside try/except. On exception, log warning + print yellow message but do NOT raise.
...
<verification>
1. `python -c "from cli.sbs_profile_init import initialize_sbs_from_wizard"` succeeds
...
5. `ruff check workspace/cli/sbs_profile_init.py workspace/__main__.py workspace/synapse_cli.py workspace/cli/onboard.py` — no lint errors
</verification>
...
- SBS questions are asked in interactive wizard after config write
- `initialize_sbs_from_wizard()` writes to linguistic, domain, and interaction layers via ProfileManager
