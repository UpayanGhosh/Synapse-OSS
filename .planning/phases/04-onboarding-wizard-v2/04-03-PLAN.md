...
must_haves:
  truths:
    ...
    - "SYNAPSE_COMMUNICATION_STYLE, SYNAPSE_INTERESTS, SYNAPSE_PRIVACY_LEVEL are the env vars"
  artifacts:
...
```
  ...
  <verify>
    <automated>cd C:/Users/upayan.ghosh/personal/Synapse-OSS/workspace && python -c "from cli.onboard import _run_non_interactive; print('non_interactive importable')" && ruff check cli/onboard.py</automated>
  </verify>
  <done>
    - `_run_non_interactive()` reads SYNAPSE_COMMUNICATION_STYLE, SYNAPSE_INTERESTS, SYNAPSE_PRIVACY_LEVEL
    ...
    - Docstring updated to document new optional env vars
...
```python
from cli.sbs_profile_init import STYLE_CHOICES, PRIVACY_CHOICES  # noqa: PLC0415
...
- `pytest tests/test_onboard.py -x -q` must all pass
  ...
  <verify>
    <automated>cd C:/Users/upayan.ghosh/personal/Synapse-OSS/workspace && pytest tests/test_onboard.py -x -q 2>&1 | tail -5</automated>
  </verify>
  <done>
...
4. `grep "SYNAPSE_INTERESTS" workspace/cli/onboard.py` — env var is referenced
5. `grep "SYNAPSE_PRIVACY_LEVEL" workspace/cli/onboard.py` — env var is referenced
</verification>

<success_criteria>
- `--non-interactive` mode accepts SYNAPSE_COMMUNICATION_STYLE, SYNAPSE_INTERESTS, SYNAPSE_PRIVACY_LEVEL
- All three env vars are optional — absent means use defaults silently
