...
must_haves:
  truths:
    ...
    - "All four test_redact.py tests (13-01-01..04) pass after this plan"
  artifacts:
    ...
    - path: "workspace/sci_fi_dashboard/observability/redact.py"
      provides: "HMAC-SHA256 salted identifier redaction with idempotency and salt bootstrap"
...
threat_model:
  ...
  stride:
    ...
    - id: "T-13-INJ-01"
      ...
      disposition: "accept"
      mitigation: "redact_identifier output is restricted to /^id_[0-9a-f]{8}$/ + literal '<none>' — cannot contain newlines, JSON delimiters, or log-injection sequences. Unit test enforces regex shape."
---
...
Output: `workspace/sci_fi_dashboard/observability/redact.py` + package `__init__.py`. Test file `workspace/tests/test_redact.py` (from Wave 0) flips from SKIP to GREEN.
</objective>
...
def redact_identifier(value: str | None) -> str:
    ...
    Returns:
      ...
      - The value unchanged IF it already matches the id_<8hex> shape (idempotency)
...
<task type="auto" tdd="true">
  <name>Task 13-01-01: Create observability package and implement redact_identifier()</name>
  ...
  <read_first>
    ...
    - workspace/tests/test_redact.py (Wave 0 — the contract this plan must satisfy)
...
def redact_identifier(value: str | None) -> str:
    ...
    Returns:
      ...
      the id_<8hex> shape (idempotency); otherwise "id_<first 8 hex chars
...
```
DELETE that line plus any blank line immediately following. The four tests now import directly and should pass against the new module.
...
```
All 4 tests MUST pass green. If a test fails, fix the implementation (not the test — tests are the contract).
  </action>
  <verify>
  ...
  <acceptance_criteria>
    ...
    - `cd workspace && pytest tests/test_redact.py -v` exits 0 with 4 passed
    - `ruff check workspace/sci_fi_dashboard/observability/` exits 0
    - `black --check workspace/sci_fi_dashboard/observability/` exits 0
  </acceptance_criteria>
  <done>Observability package exists with redact_identifier; all 4 Wave 0 tests for OBS-02 pass; ruff + black clean.</done>
</task>
...
| T-13-INJ-01 | Tampering | Log line containing redacted id | accept | Output shape `/^id_[0-9a-f]{8}$/` or literal `<none>` — no log-injection surface |
...
<verification>
1. `cd workspace && pytest tests/test_redact.py -v` — all 4 tests green
...
- All four test_redact.py tests pass
