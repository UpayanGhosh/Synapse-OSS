...
def _assert_single_run_id(
    ...
    assertions. Records with runId == '<no-run>' are IGNORED (pre-webhook
    initialization logs from other tests' residue are not assertable)."""
    run_ids = {
        p.get("runId")
        for p, _ in captured
        if p.get("runId") and p.get("runId") != "<no-run>"
    }
    if len(run_ids) == 0:
        pytest.fail("OBS-01 failure: no runId found in any captured line")
    if len(run_ids) > 1:
        pytest.fail(
...
(E) "## Check 1 — Real WhatsApp produces single-runId conversation (OBS-01)"
  ...
  - Steps: start stack → send 1 message → wait for reply → tail -n 200 ~/.synapse/logs/app.jsonl > /tmp/smoke.jsonl → jq -r '.runId' /tmp/smoke.jsonl | sort -u | grep -v '^<no-run>$' → expect ONE 12-char hex string → jq -r 'select(.runId == "RID") | .module' /tmp/smoke.jsonl | sort -u → expect at minimum: channel.whatsapp, gateway.worker, llm.router, pipeline.chat, route.whatsapp
  - Pass: all 5 modules appear; no other runId for this turn
...
All MUST pass (the full OBS-01..04 validation map from 13-VALIDATION.md).
  ...
  <done>E2E smoke test asserts all 4 invariants; per-module toggle test proves config actuation; manual validation procedure documented; full Phase 13 test map is green.</done>
</task>
...
5. Manual procedure file exists and has all 3 checks documented
</verification>

<success_criteria>
- test_single_inbound_smoke asserts all 4 OBS invariants on one real pipeline traversal
- test_per_module_level_toggle proves OBS-04 actuation at runtime
- 13-MANUAL-VALIDATION.md documents the 3 remaining manual checks with concrete jq commands
