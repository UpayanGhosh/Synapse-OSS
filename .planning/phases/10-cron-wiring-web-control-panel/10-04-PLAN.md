...
4. **test_run_cron_job_requires_auth** (DASH-02):
...
Use `TestClient` from FastAPI for the route tests. For auth tests, either configure a test gateway token or use the mock pattern from existing test files.
  ...
  <done>test_loopback_middleware.py has 5 tests covering DASH-04 (loopback enforcement, IPv6, non-dashboard routes unaffected). test_cron_routes.py has 5 tests covering DASH-02 (job listing, empty state, no service) and DASH-05 (no npm). All tests pass.</done>
</task>
...
3. `cd workspace && pytest tests/test_cron_routes.py -v` — all 5 tests pass
...
<success_criteria>
- 17 tests total across 3 test files
