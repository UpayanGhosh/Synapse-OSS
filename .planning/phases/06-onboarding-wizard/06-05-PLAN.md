...
<task type="auto">
  <name>Task 2: Provider validation tests (ONB-02, ONB-03, ONB-10) and channel/migration tests (ONB-04, ONB-05, ONB-06, ONB-08)</name>
  <files>workspace/tests/test_onboard.py</files>
  <action>
...
**8. ONB-02: All 25 providers in PROVIDER_LIST**
```python
...
def test_migration_offer_shown_when_openclaw_exists(tmp_path, monkeypatch):
    """ONB-08: Wizard mentions migration when ~/.openclaw/ exists."""
    from cli.onboard import _check_for_openclaw  # injectable helper

    fake_openclaw = tmp_path / ".openclaw"
    fake_openclaw.mkdir()
    # Direct unit test of the detection helper (no full wizard invocation needed)
...
def test_no_migration_offer_when_openclaw_absent(tmp_path):
    """ONB-08: No migration when ~/.openclaw/ does not exist."""
...
def test_github_copilot_device_flow_called(tmp_path, monkeypatch):
    ...
    # In non-interactive mode, GitHub Copilot should not attempt device flow
    # (device flow requires a browser). Test that the flow function is defined and importable.
...
Note: `_check_for_openclaw` must be an exported function from cli/onboard.py. If it is not already, the executor must add it. The function signature per the research: `def _check_for_openclaw(openclaw_root: Path | None = None) -> Path | None`. This is an intentional dependency — if Plan 04 didn't add this helper, add it when writing the test and add the function to cli/onboard.py as well.
  </action>
  <verify>
  ...
  <done>
    ...
    At least 13 test functions covering ONB-01 through ONB-10;
...
<success_criteria>
- workspace/tests/test_onboard.py exists with at least 13 test functions
