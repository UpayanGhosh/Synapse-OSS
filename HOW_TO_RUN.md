...
Your Phone (WhatsApp)
  ...
  Synapse API Gateway        ← the brain: memory, routing, persona
        ↓
  ┌──────────────────┬──────────────────────────────────────┐
  │  Gemini / Claude │  FastEmbed (ONNX, in-process)        │
  │  (cloud LLMs)    │  ← ships by default, no extra setup  │
  └──────────────────┴──────────────────────────────────────┘
        ↓
  LanceDB + SQLite           ← memory databases (embedded, zero Docker)
```

**What runs on your machine:** The Synapse API gateway (with an in-process FastEmbed ONNX model for embeddings), the Baileys WhatsApp bridge, and the databases.
**What lives in the cloud:** The LLM API you configure (Gemini, Claude, OpenRouter, etc.).

> **About embeddings:** Every message and every fact Synapse learns is converted into a
> 768-dimensional vector so retrieval is meaning-aware instead of keyword-only. The default
> provider is **FastEmbed** — an ONNX runtime that loads `nomic-embed-text-v1.5-Q` in-process.
> Zero external services, zero extra daemons. If `fastembed` is not installed, Synapse
> falls back to the **Gemini API** embedding endpoint (uses your `GEMINI_API_KEY`). You
> can pin either explicitly via `embedding.provider` in `synapse.json`.
>
> **Ollama is optional.** It is only needed if you want to run **The Vault** — a private
> conversation mode where responses are generated by a local LLM with zero cloud exposure.
> Skip the Ollama section below if you do not care about Vault.
...
| Stage | Time |
|-------|------|
| Installing prerequisites | 10–20 min (mostly downloads) |
| Downloading FastEmbed ONNX model (~100 MB, automatic on first boot) | 1–3 min |
| First-time onboarding | 5–10 min |
| Scanning WhatsApp QR code | 2 min |
| **Total** | **~20–35 min** |

> Add another 5–15 min if you also install Ollama for the optional **Vault** role.

---

## Part 1 — Install Prerequisites

You need **two things** before you start (Git + Python). Ollama is optional and only needed
for the Vault role. Docker is not required — LanceDB is embedded.
...
### 1.3 — Ollama (Optional — unlocks the Vault role)

**Skip this section if you only want standard chat.** Embeddings run in-process via
FastEmbed — no extra daemon is needed.

Install Ollama only if you want **The Vault**: a private conversation mode where responses
are generated by a local LLM with **zero cloud exposure**. The traffic cop routes messages
tagged as private to `vault` role, and `vault` uses whichever Ollama model you pick in
`synapse.json → model_mappings.vault`.
...
#### Pull a local chat model for the Vault

Pick any model Ollama supports. A small, fast default:

```bash
ollama pull llama3.2:3b          # ~2 GB, runs on 8 GB RAM
# or, smaller still:
...
Whatever you pull, reference it in `synapse.json → model_mappings.vault` as
`ollama_chat/<model>:<tag>` (for example `ollama_chat/llama3.2:3b`). The `ollama_chat/`
prefix — not `ollama/` — is required by litellm.

#### (Advanced) Using Ollama for embeddings instead of FastEmbed

Synapse v3.0 does **not** ship an Ollama embedding provider. The two default providers
are `fastembed` (local ONNX, primary) and `gemini` (cloud API, fallback). If you prefer
to run embeddings through Ollama, you would need to write a custom
`EmbeddingProvider` subclass in `workspace/sci_fi_dashboard/embedding/`. Not recommended
for first-time setup — FastEmbed is already zero-config and runs locally.
...
Dependencies ship as focused groups — install only what you need. A `uv.lock` is also
committed for reproducible installs via [uv](https://github.com/astral-sh/uv).

| File | What it installs |
|------|------------------|
| `requirements.txt` | Core runtime (FastAPI, litellm, sqlite-vec, LanceDB, FastEmbed, …) |
| `requirements-channels.txt` | WhatsApp + Telegram + Discord + Slack SDKs |
| `requirements-ml.txt` | Reranker + toxicity scorer (sentence-transformers, FlashRank) |
| `requirements-optional.txt` | Browser automation (Playwright / Crawl4AI), voice (Groq Whisper) |
| `requirements-dev.txt` | pytest, ruff, black (contributors only) |
...
pip install -r requirements-channels.txt    # any channels you will use
pip install -r requirements-ml.txt          # reranker + toxicity scorer
pip install -r requirements-optional.txt    # browser / voice (optional)
...
> **Using uv (faster, reproducible):** `uv sync` installs everything from `uv.lock` in one shot.
...
```bash
python -c "import fastapi, sqlite_vec, fastembed; print('Dependencies OK')"
```
...
> **No Ollama installed?** The example above routes `vault` to a local Ollama model. If
> you skipped Part 1.3, either (a) change `vault` to a Gemini model too — you lose air-gap
> privacy but everything else works — or (b) make sure the traffic cop never classifies
> your messages as vault (it only does so for explicitly private content).
...
Verifies Git and Python are installed and working. Ollama is checked but treated as
optional — the script notes whether it is present (for Vault support) and continues
either way. Fails with a clear message if Git or Python are missing.
...
| FastEmbed (in-process) | Embedding model (`nomic-embed-text-v1.5-Q`, ONNX) — runs inside the gateway |
| Synapse API Gateway | The brain — handles memory, routing, persona |
| Ollama (optional) | Started only if installed — powers the Vault role |
...
- Initializes the LanceDB vector store (embedded, no Docker needed)
- Starts the Synapse API Gateway, which loads FastEmbed in-process and spawns the Baileys WhatsApp bridge
- Starts Ollama only if it is installed (used for the Vault role)
...
[OK] Gateway     (8000)
[OK] LanceDB     (embedded)
[OK] FastEmbed   (in-process)
[--] Ollama      (11434)         # optional — only shown when installed
```

Gateway, LanceDB and FastEmbed must be green for Synapse to function. Ollama only needs to
be green if you configured a `vault` role that points at an Ollama model. See
[Troubleshooting](#troubleshooting) if anything is red.
...
| FastEmbed | in-process | Yes | ONNX embedder (`nomic-embed-text-v1.5-Q`) loaded inside the gateway |
| Ollama | 11434 | Optional | Only if you configure a `vault` role pointing at a local model |
...
### Embeddings are not being produced

Embeddings drive semantic memory. The default provider is **FastEmbed** (ONNX, in-process).
If it fails, Synapse falls back to the **Gemini API** when `GEMINI_API_KEY` is set.

**Check which provider the gateway picked:**

```bash
# macOS / Linux
grep -i "\[Embedding\]" ~/.synapse/logs/gateway.log | head -5
...
If you see `No embedding provider available`, run:
```bash
pip install fastembed
```

and restart the gateway. Alternatively, set `GEMINI_API_KEY` in `.env` so the Gemini
fallback can kick in.

**Pin a specific provider (advanced):** edit `~/.synapse/synapse.json` and add:
```json
"embedding": { "provider": "fastembed" }   // or "gemini"
```

---

### Ollama is not responding (Vault role only)

This only matters if you configured a `vault` role that points at an Ollama model. If you
do not use the Vault, you can ignore this section.
...
**Check that your Vault model is pulled:**
```bash
ollama list
# Whatever you put in synapse.json → model_mappings.vault must appear here.
```

If the model is missing, `ollama pull <model>` it and restart the gateway.
...
| View embedding provider | `grep "\[Embedding\]" ~/.synapse/logs/gateway.log` | `findstr "[Embedding]" "%USERPROFILE%\.synapse\logs\gateway.log"` |
| Check Ollama (Vault only) | `curl http://localhost:11434` | `curl.exe http://localhost:11434` |
| Pull a Vault model (optional) | `ollama pull llama3.2:3b` | same |
